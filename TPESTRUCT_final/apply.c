...
int isValidApply(char* definicion, TablaHash* tabla_de_funciones,TablaHash* tabla_de_listas,int* pos_lista){
    ...
    else if(isalpha(definicion[indice]) || isNumber(definicion[indice]) || definicion[indice]=='_'){
        int fin=indice;
...
DList* pasar_a_lista(char* expresion_lista,TablaHash* tabla_de_listas){
    ...
    while(partes_numericas!=NULL){
        ...
        *valor_para_lista=valor_a_agregar;
        dlist_agregar_ultimo(&estruc_lista,(void*)valor_para_lista,copysimbolic);
    ...
    while(copy[indice]!='\0' && flag){
        ...
        if(copy[indice]=='\0'){
            ...
            if(copy[indice]=='<'){
                ...
                //cuando se rompa esto ya copiamos lo q esta dentro del <> funcion
                comando[pos]='\0';
            }
            else{
                while(copy[indice]!='\0' && copy[indice]!=' '){
            ...
            if(isPrimitiva(comando)){
                ...
                if(funcion_extendida!=NULL){
                    ...
                    char limpio[1022];
                    //para saltar todos los inicios de q hay desp del <
                    int inicio=1;
                    while(comando[inicio]==' '){
                        inicio++;
                    }
                    strncpy(limpio,comando+inicio,pos-1);
                    limpio[pos-1]='\0';
                    //si no es una primitiva, ni una definida en la tabla, es porq es una del tipo <...>
                    lista_resultado=aplicar_repeticion(lista_resultado,limpio,tabla_de_funciones); 
